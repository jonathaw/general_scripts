...
Compilation info:
"The last time I compiled it was with Intel Fortran 90, version 9.  I included
the -Vaxlib option, but I don't think this is necessary any more."
 ...
 reverse translation of the protein sequences.  Having the random option present
  causes the program to choose the initial codons at random.
 ...
 Be aware that if degenerate sequences are present, all permutations of
 the degenerate sequence will be optimized.  For example, the nucleotide
 sequence

   GCGCNATCGRAACCTG

 would result in 4 (for N) * 2 (for R) = 8 unique sequences and sets of
 scores.

 The sequence can be reversed or gapfixed (see below) with additional
 directives.
